@REM f-throttle- launcher script
@REM
@REM Environment:
@REM JAVA_HOME - location of a JDK home dir (optional if java on path)
@REM CFG_OPTS  - JVM options (optional)
@REM Configuration:
@REM F_THROTTLE__config.txt found in the F_THROTTLE__HOME.
...
if "%F_THROTTLE__HOME%"=="" (
  ...
  set "F_THROTTLE__HOME=%~dp0\\.."
) else (
  set "APP_HOME=%F_THROTTLE__HOME%"
)
...
set "CFG_FILE=%APP_HOME%\F_THROTTLE__config.txt"
set CFG_OPTS=
...
set "APP_MAIN_CLASS=io.fth.Report"
...
if "%JAVAOK%"=="false" (
  ...
  echo and download a valid Java JDK and install before running f-throttle-.
  echo.
...
"%_JAVACMD%" !_JAVA_OPTS! !F_THROTTLE__OPTS! -cp "%APP_CLASSPATH%" %MAIN_CLASS% !_APP_ARGS!
